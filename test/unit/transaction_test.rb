...
class TransactionTest < ActiveSupport::TestCase
  ...
  test "simple transaction with one participant" do
    Transaction.create! :name => "transaction1", :creditor => User.find(1).id, :debtors => [User.find(4).id], :amount => 10
    assert_equal 0,  User.find(1).owes(User.find(4))
    assert_equal 10, User.find(4).owes(User.find(1)).to_i

    Transaction.create! :name => "transaction2", :creditor => User.find(4).id, :debtors => [User.find(1).id], :amount => 5.5
  ...
  test "split bill" do
    Transaction.create! :name => "transaction1", :creditor => 1, :debtors => [2,3,4], :amount => 9
  ...
  test "split bill with uneven result 1" do
    Transaction.create! :name => "transaction1", :creditor => 1, :debtors => [2,3,4], :amount => 7.8
  ...
  test "split bill with uneven result 2" do
    Transaction.create! :name => "transaction1", :creditor => 1, :debtors => [2,3,4], :amount => 7.9
